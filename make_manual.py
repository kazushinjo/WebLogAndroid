...
add_para('バージョン 1.09（2026-05-14 更新）　対応機種: Android 8.0 (API 26) 以上')
doc.add_paragraph()
...
add_heading('入力の流れ', 2)
for s in [
    '1. コールサインを入力（半角英数字のみ・自動的に大文字になります）',
    '2. 日付を入力（YYYYMMDD 形式　例: 20250509　→ 2025/05/09 と表示されます）',
    '3. 時刻を入力（HHMM 形式・JST　例: 1023　→ 10:23 と表示されます）',
...
add_para('・JCC コード欄は半角英数字のみ入力できます', indent=True)
...
add_table(
    ...
    [
        ('2026-05-14 v1.09',
         '・コールサイン欄を半角英数字のみ受け付けに制限（日本語IME無効化）\n'
         '・JCCコード欄を半角英数字のみ受け付けに制限（日本語IME無効化）\n'
         '・日付欄: YYYYMMDD 入力・YYYY/MM/DD 表示\n'
         '・時刻欄: HHMM 入力・HH:MM 表示'),
        ('2026-05-13 v1.08',
         '・初版リリース（Android版）\n'
         '・ボタンのコントラスト改善（白文字に修正）\n'
...
run = p.add_run('WebLog for Android　操作説明書　v1.09　© 2026 JH1XHX/JA6FUF')
run.font.size = Pt(9)
set_font(run)
